...
function InteractiveRack({ tiles, onClick, selecting, selected, highlighted }) {
  ...
  return (
    <div className={selecting ? "rack selecting" : "rack"}>
...
export function Rack({ tiles, highlighting, highlighted = new Set() }) {
  highlighted = new Set(highlighted);
  return (
    <div className={highlighting > 0 ? "rack selecting" : "rack"}>
      {tiles.flat().reduce((elems, tile, index) => {
        return [
          ...elems,
          <span
            className={highlighted.delete(tile) ? "tile selected" : "tile"}
            data-tile={tile}
            key={tile + index}
          />,
        ];
...
function Actions({
  ...
  if (pendingAction === "" || pendingAction === "continue") {
    return (
      <>
        <button disabled={!canDraw} onClick={doDraw} id="buttonDraw">
          {timeUntilDraw > 0 ? timeUntilDraw : "Draw tile"}
        </button>
        <button disabled={!canChow} onClick={doChow} id="buttonChow">
          {timeUntilDraw > 0 ? timeUntilDraw : "Chi"}
        </button>
        <button disabled={!canPeng} onClick={doPeng} id="buttonPeng">
          Peng
        </button>
        <button disabled={!canKong} onClick={doKong} id="buttonKong">
          Kong
        </button>
        <button disabled={!canHu} onClick={doHu} id="buttonHu">
          Declare win
        </button>
        {canEndGame && (
          <button onClick={doEndGame} id="buttonEnd">
            End game in draw
          </button>
        )}
      </>
...
function Board({ nonce, seat, players, round, doAction }) {
    ...
    if (currentTurn === seat && currentAction === mahjong.ACTION_DRAW) {
      if (lastDiscardTime + pongDuration - Date.now() > 0) {
        const timer = setTimeout(() => {
          setTimeUntilDraw(
            Math.round((lastDiscardTime + pongDuration - Date.now()) / 100) / 10
          );
        }, 100);
  ...
  const canDiscard =
    round.draws_left > 0 &&
    currentTurn === seat &&
    currentAction === mahjong.ACTION_DISCARD;
  const canDraw =
    !waitingForPong &&
    round.draws_left > 0 &&
    currentTurn === seat &&
    currentAction === mahjong.ACTION_DRAW;
  const canChow = discards.length > 0 && canDraw;
  const canPeng =
    currentAction === mahjong.ACTION_DRAW &&
    seat !== previousTurn &&
    mahjong.canPeng(self.concealed || [], discards[discards.length - 1]);
  const canKongFromDiscard =
    discards.length > 0 &&
    seat !== previousTurn &&
    currentAction === mahjong.ACTION_DRAW;
  const canKongFromHand =
    seat === currentTurn && currentAction === mahjong.ACTION_DISCARD;
  ...
  if (pendingAction === "continue") {
    ...
    message =
      'Try to win, or use the "End game in draw" button to end the game in a draw';
  }
  ...
  const selectTile = async (tile, index) => {
    if (pendingAction === "chow") {
      ...
      if (selected.size === 2) {
        doAction(
          "chow",
          [...selected].map((i) => self.concealed[i])
        );
        setSelected(new Set());
        setPendingAction("");
      } else {
    ...
    <>
      <div className="table" onClickCapture={tableClick}>
        ...
        <div className="bottom">
          <Rack
            tiles={self.flowers}
            highlighting={pendingAction === "continue"}
            highlighted={highlightedFlowers}
          />
          <Rack
            tiles={self.revealed.concat(melds)}
            highlighting={pendingAction === "continue"}
          />
          <InteractiveRack
            tiles={pendingAction === "win" ? remaining : self.concealed}
            onClick={selectTile}
            selecting={
              pendingAction === "chow" ||
              pendingAction === "win" ||
        ...
        <div className="actions">
          <div>
            <Actions
              ...
              doDraw={drawTile}
              canChow={canChow}
              doChow={selectTilesForChow}
              canPeng={canPeng}
              doPeng={pengTile}
              canKong={canKong}
              doKong={selectTilesForKong}
              canHu={canHu}
              doHu={declareWin}
